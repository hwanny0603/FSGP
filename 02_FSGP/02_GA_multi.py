...
def read_data(gene,targets,queue):
    ...
    ##### Save result #####
    outfile = open("/storage/home/sehwanahn/FSGP/research/result/FSGP.txt",'a')
    outfile.write(gene + "\t" + str(result[0]) + "\t" + str(result[1]) + "\t" + str(s_cell) + "\n")
...
no_generation = 100
no_fitness = 20
no_mating = int((no_generation - no_fitness)/2)	# no_fitness + no_mating*2 = no_generation 
ratio_crossover = .7
...
def main():
    ...
    for f in files:
        ...
        job = pool.apply_async(read_data,(gene,targets,queue))
        jobs.append(job)
    for job in jobs:
        job.get()
